...
export const useLongestYAxisLabel = (dataset, elements, legendPosition) => useMemo(() => {
    let labelLength = 0;
    const primaryElement = elements[0];
    if (dataset instanceof Array && primaryElement) {
        const resolveAllMeasureLabels = (item) => {
            return elements.map((elementConfig) => primaryElement.formatter(getValueByDataKey(item, elementConfig.accessor, '')));
        };
